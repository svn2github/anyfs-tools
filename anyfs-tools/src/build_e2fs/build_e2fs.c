/*
 * build_e2fs.c - Build a ext2fs filesystem using external anyfs inode table
 * Copyright (C) 2005-2006 Nikolaj Krivchenkov aka unDEFER <undefer@gmail.com>
 * 
 * BASED ON mke2fs from e2fsprogs, Copyright (C) 1994-2005 by Theodore Ts'o.
 * The most functions wasn't changed.
 *
 * %Begin-Header%
...
/* Usage: build_e2fs [options] inode_table device
 * 
...
const char * program_name = "build_e2fs";
const char * device_name /* = NULL */;
const char * inode_table /* = NULL */;
...
static void usage(void)
{
	...
	"[-r fs-revision] [-R options] [-qvSV] inode_table device [blocks-count]\n"
...
{
	return param.s_blocks_count;
}

static void PRS(int argc, char *argv[], struct any_sb_info **pinfo)
{
	int		b, c;
	int		size;
	char *		tmp;
...
#define _SC_PAGESIZE _SC_PAGE_SIZE
	...
	if ( ( (argc - optind) != 2 ) && !show_version_only)
		usage();

	if (!quiet || show_version_only)
		fprintf (stderr, "build_e2fs %s (%s)\n", ANYFSTOOLS_VERSION, 
			 ANYFSTOOLS_DATE);
	...
	retval = read_it (pinfo, (char*) inode_table);
	if (retval) 
	{
		fprintf(stderr,
...
(errno)?strerror(errno):_("bad format"));
	...
	{
		char *lost_found = strdup("/lost+found");
		mkpathino(lost_found, 1, *pinfo, NULL);
		free(lost_found);
	}

	if ( blocksize!=-1 && blocksize!=(*pinfo)->si_blocksize )
	{
		fprintf(stderr,
...
/*
 * As known, ext2fs has 12 direct links to block of inode,
 * one indirect links,
 * one 2 degree indirect links,
 * and 3 degree indirect links,
 *
 * so this function calculate number of block in each of 4 arrays
 * with direct or indirect links to block of inode.
 */
void get_num_block(unsigned long block, 
		unsigned long blocksize, long r[4]) 
{
	blocksize/=4;
	int i, j;
...
int set_buffer( ext2_filsys fs, struct ind_block *ind_block,
	       	unsigned long block )
{
	int retval;

	if (ind_block->block!=block)
	{
		if (ind_block->dirty || ind_block->block>0)
		{
			if (!noaction)
			{
				retval = io_channel_write_blk(fs->io, ind_block->block,
						1, ind_block->buffer);
				if (retval)
					fprintf(stderr,
		...
		ind_block->block = block;
		
		if (ind_block->block>0)
		{
...
int set_block (ext2_filsys fs,
	       	struct ind_block ind_block[3], 
		struct ext2_inode *inode, unsigned long i_block,
		unsigned long block)
{
	long blocks[4];
	int clean, retval;
	get_num_block(i_block,
			fs->blocksize,
			blocks);
	...
	if ( blocks[1]<0 )
	{
		...
		set_buffer(fs, ind_block, inode->i_block[blocks[0]]);

		if (clean)
			memset(ind_block[0].buffer, 0, fs->blocksize);

		if ( blocks[2]<0 )
		{
			ind_block[0].buffer[blocks[1]] = block;
			ind_block[0].dirty = 1;
		}
		else
		{
			...
			set_buffer(fs, ind_block+1, ind_block[0].buffer[blocks[1]]);

			if (clean)
				memset(ind_block[1].buffer, 0, fs->blocksize);

			if ( blocks[3]<0 )
			{
				ind_block[1].buffer[blocks[2]] = block;
				ind_block[1].dirty = 1;
			}
			else
			{
				...
				set_buffer(fs, ind_block+2, ind_block[1].buffer[blocks[2]]);

				if (clean)
					memset(ind_block[2].buffer, 0, fs->blocksize);

				ind_block[2].buffer[blocks[3]] = block;
				ind_block[2].dirty = 1;
			}
...
int main (int argc, char *argv[])
{
	...
	if (fs->blocksize!=info->si_blocksize)
	{
		fprintf(stderr, _("%lu is Bad BlockSize (Good BlockSize is %d)\n"),
				info->si_blocksize, fs->blocksize);
		exit(1);
	}
	...
	if (verbose)
		printf (_("Releasing blocks of system info successful\n"));

	for (i=1; i<param.s_blocks_count; i++)
		if ( test_bit( i, block_bitmap) )
			ext2fs_mark_block_bitmap(fs->block_map, i);
	
	for (i=0; i<info->si_inodes; i++)
		if ( test_bit( i, info->si_inode_bitmap) ) {
			
			unsigned long e_ino = (i==1)?EXT2_ROOT_INO:
				EXT2_FIRST_INODE(fs->super) + i - 2;
			
			ext2fs_mark_inode_bitmap(fs->inode_map, e_ino);
		}
	...
	if (!noaction)
	{
		...
		for (i=1; i < min_t(unsigned, param.s_blocks_count, 128*1024/fs->blocksize); i++)
		{
			if ( ! ext2fs_test_block_bitmap(fs->block_map, i) )
			{
				retval = io_channel_write_blk(fs->io, i,
						1, buffer);
				if (retval)
					fprintf(stderr,
							_("Warning: could not write block #%ld: %s\n"),
	...
	if (super_only) {
		...
		if (start > 0 && !noaction)
		{
			...
			for (i=0; i < blocks - start; i++)
			{
				if ( ! ext2fs_test_block_bitmap(fs->block_map, start + i) )
				{
					retval = io_channel_write_blk(fs->io, start + i,
							1, buffer);
					if (retval)
						fprintf(stderr,
								_("Warning: could not write block #%ld: %s\n"),
		...
		if (verbose)
			printf (_("Starting building ext2fs filesystem\n"));
		...
		for (i=0; i<info->si_inodes; i++) {
			...
			static struct ind_block ind_blocks[3] =
			...
			for (ii=0; ii<3; ii++) 
			{
				if (!ind_blocks[ii].buffer)
				{
					ind_blocks[ii].buffer =
						malloc (fs->blocksize);
					if (!ind_blocks[ii].buffer)
					{ retval = -ENOMEM; goto out; }
				}
			...
			inode.i_links_count = info->si_inode_table[i].i_links_count;
			inode.i_blocks = 0;
			...
			if ( S_ISLNK(info->si_inode_table[i].i_mode) ) {
				...
				retval = set_block(fs, ind_blocks, &inode,
						inode.i_blocks, block);
				if (retval<0) 
					goto out;

				num_allocated_ind_blocks+=retval;
				inode.i_blocks++;

				strcpy(buffer, info->si_inode_table[i].i_info.symlink);

				if (!noaction)
				{
					retval = io_channel_write_blk(fs->io, block,
							1, buffer);
					if (retval)
						fprintf(stderr,
								_("Warning: could not write block #%ld: %s\n"),
			...
			if ( S_ISREG(info->si_inode_table[i].i_mode) ) {
						...
						file_frags->fr_nfrags; j++)
				{
					...
					unsigned long length=
						...
						fr_length;
					if (verbose>=3)
						printf (_("fragment: %lu from %u (length = %lu)\n"), j,
								info->si_inode_table[i].i_info.
								file_frags->fr_nfrags, length);
				...
				set_buffer(fs, ind_blocks+1, 0);
				set_buffer(fs, ind_blocks+2, 0);
			} else

			if ( S_ISDIR(info->si_inode_table[i].i_mode) ) {
				unsigned long buffer_offset = 0;
				struct ext2_dir_entry_2   *edir = NULL;
				struct ext2_dir_entry_2   *lastedir = NULL;
				...
				for (; dirent; dirent = dirent->d_next) {
					...
					if ((buffer_offset+rec_len)>=
							fs->blocksize)
					{
						if (lastedir)
						{
							lastedir->rec_len +=
								fs->blocksize - buffer_offset;
						}
					...
					edir = (struct ext2_dir_entry_2*)
						(buffer + buffer_offset);
					edir->inode = (dirent->d_inode==1)?EXT2_ROOT_INO:
						EXT2_FIRST_INODE(fs->super) + dirent->d_inode - 2;
					strcpy(edir->name, dirent->d_name);
					edir->name_len = name_len;
					edir->rec_len = rec_len;

					unsigned d_type = info->si_inode_table[dirent->d_inode].i_mode >> 12;
					edir->file_type = EXT2_FT_MAX-1;
					while ( edir->file_type > EXT2_FT_UNKNOWN && ext2_filetype_table[edir->file_type] != d_type )
						edir->file_type--;

					buffer_offset += edir->rec_len;
					lastedir = edir;
				}
				...
				inode.i_size = inode.i_blocks * fs->blocksize;
			} else
			{
				unsigned char minor;
				unsigned char major;
				...
				major = info->si_inode_table[i].
					i_info.device>>20 & 0xFF;
				inode.i_block[0] = major<<8 | minor;
			}

			inode.i_blocks += num_allocated_ind_blocks;
			inode.i_blocks *= fs->blocksize/512;

			if (!noaction)
				ext2fs_write_inode (fs, e_ino, &inode);
		}
	...
	{
		dgrp_t 	group;
		int 	group_inode, i, ino;
		blk_t   group_block, n, blk;
		int 	numblocks, numinodes;

		group_block = fs->super->s_first_data_block;
		group_inode = 1;

		fs->super->s_free_blocks_count = 0;
		fs->super->s_free_inodes_count = 0;
		...
		for (group = 0; group < fs->group_desc_count; group++) {
			...
			if (group == fs->group_desc_count-1) {
				numinodes = fs->super->s_inodes_count %
					fs->super->s_inodes_per_group;
				if (!numinodes)
					numinodes = fs->super->s_inodes_per_group;
			} else          
				numinodes = fs->super->s_inodes_per_group;
			...
			group_block += fs->super->s_blocks_per_group;
			group_inode += fs->super->s_inodes_per_group;
		}
	}
	...
	if (journal_device) {
		...
		if (noaction)
		{
			...
			{
				retval = ext2fs_new_block(fs, block, 0, &block);
				if (retval) { retval = -ENOSPC; goto out; }
